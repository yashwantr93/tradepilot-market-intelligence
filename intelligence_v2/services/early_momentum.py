"""
Early Momentum orchestration service.

Provides the five services required by Phase 3:
  - Early Momentum calculation -> run_momentum_backfill() / calculate_for_date()
  - Ranking                     -> _rank_within_categories() (within-category only)
  - History retrieval           -> get_symbol_history() / get_category_history()
  - Category assignment         -> delegates to processors.momentum_classifier (pure)
  - Reason generation           -> delegates to processors.momentum_classifier (pure)
...
log = get_v2_logger("services.early_momentum")
...
def _symbol_to_sector() -> dict[str, str]:
    ...
    is alphabetical so the mapping is deterministic and reproducible.
    """
...
def _load_sector_context(trade_date: dt.date) -> tuple[dict[str, str], dict[str, str]]:
    """(sector -> Phase 1 state, sector -> Phase 2 cycle stage) for one date."""
    with session_scope() as s:
        states = {
            r.sector: r.state for r in s.scalars(
...
def _load_v1_opportunity_symbols(as_of: dt.date) -> set[str]:
    """Symbols flagged by V1's existing Opportunity filter (daily_watchlist)
    within the lookback window — read-only reuse of an existing V1 signal."""
    since = as_of - dt.timedelta(days=V1_OPPORTUNITY_LOOKBACK_DAYS)
    df = read_v1(
        "SELECT DISTINCT symbol FROM daily_watchlist "
        "WHERE trade_date <= :as_of AND trade_date >= :since",
...
def _rank_within_categories(rows: list[dict]) -> list[dict]:
    ...
    Sort keys: signal_count desc, rs_1m desc, symbol asc. There is deliberately
    no market-wide ranking — ranks are only comparable within one category.
    """
    ranked: list[dict] = []
    for category in CATEGORIES:
        cohort = [r for r in rows if r["category"] == category]
        cohort.sort(key=lambda r: (-r["signal_count"],
...
def calculate_for_date(as_of: dt.date, series: dict, nifty_closes: pd.Series,
    ...
    sector_states, cycle_stages = _load_sector_context(as_of)
    v1_opportunity = _load_v1_opportunity_symbols(as_of)
    ...
    for symbol, frame in series.items():
        ...
        signals = evaluate_signals(metrics, sector_state, cycle_stage, symbol in v1_opportunity)
...
def get_available_dates() -> list[dt.date]:
    ...
    so Phase 2's dates are the binding constraint)."""
    with session_scope() as s:
...
def run_momentum_backfill(dates: list[dt.date] | None = None) -> dict:
    ...
    log.info("Early Momentum backfill: %d dates, %d rows, universe %d symbols",
