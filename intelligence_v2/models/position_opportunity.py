"""
Position Opportunity — append-only daily stock classification table.

One row per (symbol, trade_date). Mirrors `EarlyMomentumDaily`/
`BearishOpportunityDaily`'s shape (same metric fields, since all three are
computed by the same shared `momentum_metrics.compute_stock_metrics()`),
storing the category, every signal's boolean outcome, the human-readable
reasons, the within-category rank, and the raw metrics behind them so any
classification can be audited without recomputation.
"""
...
class PositionOpportunityDaily(Base):
    __tablename__ = "position_opportunity_daily"
    __table_args__ = (UniqueConstraint("trade_date", "symbol", name="uq_position_date_symbol"),)
    ...
    # Context (from Phase 1 / Phase 2 / Phase 3 / V1 symbol master)
    sector: Mapped[str | None] = mapped_column(String)
    sector_state: Mapped[str | None] = mapped_column(String)
    cycle_stage: Mapped[str | None] = mapped_column(String)
    early_momentum_category: Mapped[str | None] = mapped_column(String)
