"""
Early Momentum — append-only daily stock classification table.

One row per (symbol, trade_date). Stores the category, every signal's boolean
outcome, the human-readable reasons, the within-category rank, and the raw
metrics behind them so any classification can be audited without recomputation.
"""
...
class EarlyMomentumDaily(Base):
    __tablename__ = "early_momentum_daily"
    __table_args__ = (UniqueConstraint("trade_date", "symbol", name="uq_momentum_date_symbol"),)
    ...
    # Metrics (computed from V1 price_history — no new indicators)
