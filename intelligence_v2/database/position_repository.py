"""
Repository for `position_opportunity_daily` — the only place that table is
read or written. Writes are idempotent (replace-for-date).
"""
...
from intelligence_v2.models import PositionOpportunityDaily
...
def replace_for_date(trade_date: dt.date, rows: list[dict]) -> int:
    ...
    with session_scope() as s:
        s.execute(delete(PositionOpportunityDaily).where(
            PositionOpportunityDaily.trade_date == trade_date))
        for r in rows:
            s.add(PositionOpportunityDaily(**r))
    return len(rows)


def get_by_date(trade_date: dt.date, category: str | None = None) -> pd.DataFrame:
    with session_scope() as s:
        stmt = select(PositionOpportunityDaily).where(PositionOpportunityDaily.trade_date == trade_date)
        if category:
            stmt = stmt.where(PositionOpportunityDaily.category == category)
        rows = s.scalars(stmt.order_by(PositionOpportunityDaily.rank_in_category)).all()
    return _to_df(rows)
...
def get_latest(category: str | None = None) -> pd.DataFrame:
    with session_scope() as s:
        latest = s.scalar(
            select(PositionOpportunityDaily.trade_date)
            .order_by(PositionOpportunityDaily.trade_date.desc()).limit(1))
    if latest is None:
        return pd.DataFrame()
...
def get_symbol_history(symbol: str) -> pd.DataFrame:
    with session_scope() as s:
        rows = s.scalars(
            select(PositionOpportunityDaily)
            .where(PositionOpportunityDaily.symbol == symbol)
            .order_by(PositionOpportunityDaily.trade_date.asc())
        ).all()
    return _to_df(rows)
...
def get_distinct_dates() -> list[dt.date]:
    with session_scope() as s:
        return list(s.scalars(
            select(PositionOpportunityDaily.trade_date).distinct()
            .order_by(PositionOpportunityDaily.trade_date)).all())


def get_category_counts_by_date() -> pd.DataFrame:
    """Category counts per date — powers the History tab's trend view."""
    with session_scope() as s:
        rows = s.scalars(select(PositionOpportunityDaily)).all()
    if not rows:
    ...
    return pd.DataFrame([
        {
            ...
            "early_momentum_category": r.early_momentum_category,
            ...
            "dist_52w_high_pct": r.dist_52w_high_pct, "sector_rs_1m": r.sector_rs_1m,
