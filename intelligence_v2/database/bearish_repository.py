"""
Repository for `bearish_opportunity_daily` — the only place that table is read
or written. Writes are idempotent (replace-for-date).
"""
...
from intelligence_v2.models import BearishOpportunityDaily
...
def replace_for_date(trade_date: dt.date, rows: list[dict]) -> int:
    ...
    with session_scope() as s:
        s.execute(delete(BearishOpportunityDaily).where(
            BearishOpportunityDaily.trade_date == trade_date))
        for r in rows:
            s.add(BearishOpportunityDaily(**r))
    return len(rows)


def get_by_date(trade_date: dt.date, category: str | None = None) -> pd.DataFrame:
    with session_scope() as s:
        stmt = select(BearishOpportunityDaily).where(BearishOpportunityDaily.trade_date == trade_date)
        if category:
            stmt = stmt.where(BearishOpportunityDaily.category == category)
        rows = s.scalars(stmt.order_by(BearishOpportunityDaily.rank_in_category)).all()
    return _to_df(rows)
...
def get_latest(category: str | None = None) -> pd.DataFrame:
    with session_scope() as s:
        latest = s.scalar(
            select(BearishOpportunityDaily.trade_date)
            .order_by(BearishOpportunityDaily.trade_date.desc()).limit(1))
    if latest is None:
        return pd.DataFrame()
...
def get_symbol_history(symbol: str) -> pd.DataFrame:
    with session_scope() as s:
        rows = s.scalars(
            select(BearishOpportunityDaily)
            .where(BearishOpportunityDaily.symbol == symbol)
            .order_by(BearishOpportunityDaily.trade_date.asc())
        ).all()
    return _to_df(rows)
...
def get_distinct_dates() -> list[dt.date]:
    with session_scope() as s:
        return list(s.scalars(
            select(BearishOpportunityDaily.trade_date).distinct()
            .order_by(BearishOpportunityDaily.trade_date)).all())


def get_category_counts_by_date() -> pd.DataFrame:
    """Category counts per date — powers the History tab's trend view."""
    with session_scope() as s:
        rows = s.scalars(select(BearishOpportunityDaily)).all()
    if not rows:
